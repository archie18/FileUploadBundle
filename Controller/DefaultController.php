...
class DefaultController extends AbstractController
{
     ...
     */
    public function downloadAction($id, $mapping, AuthorizationCheckerInterface $securityContext,TranslatorInterface $translator) {
...
//        var_dump($this->getParameter('melolab_biogestion_fileupload.mappings'));
//        var_dump($this->getParameter('vich_uploader.mappings')); die();
        
        $mappings = $this->getParameter('melolab_biogestion_fileupload.mappings');
        $config = $mappings[$mapping];

        $entity = $this->getDoctrine()->getManager()->getRepository($config['entity'])->{$config['repository_method']}($id);

        if (!$entity) {
            throw $this->createNotFoundException($translator->trans('file.entity_not_found'));
        }
        
        // Security
        if (true === $config['allow_anonymous_downloads']) {
            if (true === $securityContext->isGranted('IS_AUTHENTICATED_REMEMBERED') and false === $securityContext->isGranted('VIEW', $entity)) {
        ...
        if (!$filename) {
            throw $this->createNotFoundException($translator->trans('file.file_not_found'));
        }
        
        $ext = pathinfo($filename, PATHINFO_EXTENSION);
        $uploadFolder = $this->getParameter('vich_uploader.mappings')[$config['vich_mapping']]['upload_destination'];
...
//        }
        ...
        $ext = pathinfo($filename, PATHINFO_EXTENSION);
        $uploadFolder = $this->getParameter('melolab_biogestion_fileupload.temp_files_path');
...
//            var_dump($form->get('temp_filename')->getData());
            ...
            $entityClass = $this->getParameter('melolab_biogestion_fileupload.mappings')[$mapping]['entity'];
            $fileField = $this->getParameter('melolab_biogestion_fileupload.mappings')[$mapping]['file_field'];
            $fileSetterMethod = $this->getParameter('melolab_biogestion_fileupload.mappings')[$mapping]['file_setter'];
            $fileNameSetterMethod = $this->getParameter('melolab_biogestion_fileupload.mappings')[$mapping]['filename_setter'];

            if($form->get('temp_filename')->getData() !== null){

                $tempFolder = $this->getParameter('melolab_biogestion_fileupload.temp_files_path');
